...
from utils.utils import set_seed, new_length, downsample_torch, get_accuracy, create_dir, create_file, refurb_label, count_refurb_matrix
...
parser.add_argument('--alpha', type=float, default=0.001, help='MCR hyperparameters')
...
def train(epoch, train_loader, train_dataset, train_target, encoders, classifiers, optimizer, refurb_matrixs,
    ...
    if epoch >= args.start_refurb:
        train_target, unselected_inds, update_inds = refurb_label(train_loader, train_target, [refurb_matrix1, refurb_matrix2, refurb_matrix3], unselected_inds, update_inds)
        # reload train loader
        train_loader = load_loader(args, train_dataset, train_target)

    return epoch_train_loss, epoch_train_acc, train_loader, train_target, [encoder1, encoder2, encoder3], [classifier1, classifier2, classifier3], [refurb_matrix1, refurb_matrix2, refurb_matrix3], [k_train_loss1, k_train_loss2, k_train_loss3], unselected_inds, update_inds
...
def main():
    ...
    for epoch in range(args.epoch):
        # train
        train_loss, train_acc, train_loader, train_target, encoders, classifiers, refurb_matrixs, k_train_losses, unselected_inds, update_inds = (
