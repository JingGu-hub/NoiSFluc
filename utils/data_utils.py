...
def load_loader(args, dataset, target, shuffle=True):
    train_set = TimeDataset(torch.from_numpy(dataset).type(torch.FloatTensor).cuda(),
                            torch.from_numpy(target).type(torch.LongTensor).cuda())
    train_loader = DataLoader(train_set, batch_size=args.batch_size, num_workers=args.num_workers, shuffle=shuffle)
